from google import genai
from google.genai import types
import PIL.Image

# ---------------------------------------------------------
# 1. API 키 설정 (본인의 키를 넣어주세요)
# ---------------------------------------------------------
import os

# [수정 전]
# API_KEY = "AIzaSyDEYJtUQpD5sFSCfUieDyinP45C_tq_xbM"

# [수정 후] 소스코드에는 키를 없애고, 환경변수에서 가져오도록 변경
API_KEY = os.environ.get("GEMINI_API_KEY") 

if not API_KEY:
    raise ValueError("API Key가 설정되지 않았습니다!")
# 2. 클라이언트 생성 (이게 새로운 방식입니다!)
client = genai.Client(api_key=API_KEY)
print("🔍 사용 가능한 모델 목록:")
try:
    # 복잡한 필터링 다 빼고 이름만 출력!
    for m in client.models.list():
        print(f"- {m.name}")
except Exception as e:
    print(f"❌ 에러: {e}")
# ---------------------------------------------------------
# 3. 이미지 불러오기
# ---------------------------------------------------------
image_path = "poster.jpg"  # 이미지 파일명 확인!

try:
    img = PIL.Image.open(image_path)
    print(f"✅ 이미지 로드 성공: {image_path}")
except FileNotFoundError:
    print("❌ 이미지를 찾을 수 없습니다. 파일명을 확인해주세요.")
    exit()

# ---------------------------------------------------------
# 4. API 콜 날리기 (Gemini 1.5 Flash)
# ---------------------------------------------------------
print("🤖 AI에게 질문을 보내는 중입니다...")

try:
    response = client.models.generate_content(
    model="gemini-flash-lite-latest",
        contents=[
            "이 이미지는 학교 공지사항 포스터야. 핵심 내용을 3줄로 요약해주고, 분류(채용/공모전/행사)를 해줘. 일시와 장소, 키워드도",
            img
        ]
    )
    
    # 5. 결과 출력
    print("\n" + "="*30)
    print("📢 AI의 분석 결과")
    print("="*30)
    print(response.text)

except Exception as e:
    print(f"❌ 에러 발생: {e}")
